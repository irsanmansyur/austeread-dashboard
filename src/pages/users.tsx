...
import ButtonChangePassword from "@/components/form/buttonChangePassword";
import ChangePasswordUser from "@/components/user/changePasswordUser";

function UserList() {
  const { useAxios } = useAuth();
  const [{ data = { data: [] }, loading, error }, refetchUsers] = useAxios<{ data: AppInterface.User[] }>({
    url: "/users",
    method: "GET",
  });
                      ...
                      <tr key={user.id}>
                        ...
                        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                          <div className=" flex justify-center items-center gap-2">
                            <ButtonEdit onClick={(e) => setUserEdit(user)}>Edit</ButtonEdit>
                            <ButtonChangePassword onClick={(e) => setUserChangePassword(user)}>Change</ButtonChangePassword>

                            <DeleteUser
                              user={user}
                              onDeleted={(status) => {
                                if (status == true) refetchUsers();
                              }}
                            />
                          </div>
                        </td>
