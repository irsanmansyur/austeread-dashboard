...
export function ListProduct() {
  ...
  const [{ data = { data: [] }, loading, error }, refetchData] = useAxios<{ data: AppInterface.Product[] }>({ url: "/productsShop" });

  return (
    <HelmetLayout title="Product List">
      <div className="py-8">
        <div className="flex justify-between">
          <h2 className="text-2xl font-semibold leading-tight">Product List</h2>
          <Link to={routes.product.add} className="flex items-center gap-2 cursor-pointer">
            Tambah
          </Link>
        </div>
                  ...
                  data.data.map((product, i) => {
                    return (
                      <tr key={product.id}>
                        ...
                        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">{"Creator -"}</td>
                        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">{product.createdAt}</td>
                        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                            ...
                            <ButtonDelete
                              handleDelete={async (property) => {
                                return api
                                  ...
                                  .then((resp) => refetchData())
                                  .catch((error) => {
                                    property.setError(error?.message || "Gagal hapus Product");
                                  });
                              }}
                            >
                              Delete
