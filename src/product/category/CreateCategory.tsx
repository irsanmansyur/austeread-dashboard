...
export default function CreateCategory({ open = false, onCLose }: Props) {
  let [isOpen, setIsOpen] = useState(open);
  const { useAxios, user } = useAuth();
  const [{ loading: postLoading, error: postError, response: responsePost }, executePost] = useAxios({ url: "/products-category", method: "POST" }, { manual: true });
