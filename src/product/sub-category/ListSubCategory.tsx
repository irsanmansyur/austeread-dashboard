...
export function SubCategoryProductList() {
  ...
  const [subCategoryEdit, setSubCategoryEdit] = useState<AppInterface.SubProductCategory>();
  const [subCategoryCreate, setSubCategoryCreate] = useState(false);

  return (
    <HelmetLayout title="List   Product sub-category">
      <div className="py-8">
        <div className="flex justify-between">
          <h2 className="text-2xl font-semibold leading-tight">Product sub category</h2>
          <div className="flex items-center gap-2 cursor-pointer" onClick={(e) => setSubCategoryCreate(true)}>
                      ...
                      <tr key={category.id}>
                        ...
                        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                          <div className=" flex justify-center items-center gap-2">
                            <ButtonEdit onClick={(e) => setSubCategoryEdit(category)}>Edit</ButtonEdit>
                            <ButtonDelete
                              handleDelete={async (property) => {
                                return api
                                  .delete(`products-subcategory/${category.id}`)
                                  .then((resp) => refetchCategory())
                                  .catch((error) => {
                                    property.setError(error?.message || "Gagal hapus SubCategory");
                                  });
                              }}
                            >
                              Delete
      ...
      <SubCategoryEdit
        ...
        onCLose={(updated) => {
          setSubCategoryEdit(undefined);
          updated && refetchCategory();
        }}
      />
      <CreateSubCategory
        open={subCategoryCreate}
        onCLose={(updated) => {
